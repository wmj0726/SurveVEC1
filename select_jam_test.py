...
utilitys = np.zeros((6, max_time_slot))
save_energy = np.zeros((6, max_time_slot))
save_sinr = np.zeros((6, max_time_slot))
save_power = np.zeros((6, max_time_slot))




def initialize_select_state_seq(episode, hacking_car_id, jam_num):
    ...
    save_power1 = []  # np.zeros((8, max_time_slot))
    jam_power = [5]*jam_num
    jam_select_dis = np.zeros(jam_num)
    jam_gain = np.zeros(jam_num)
    ...
    for i in range(len(rsu)):  # rsu数量
        ...
        for j in range(rsu_time_slot):
            utility, delay, energy, sinr, h_sinr, R = vehicle.sim_calculate_utility(offload_rate=offload_rate,
                                                                         ...
                                                                         jam_power=jam_power, jam_gain=jam_gain, rsu_h_gain=rsu_h_gain, noise=noise, k=jam_num)
...
def get_jam_to_h_sinr(jam_power, jam_gain, noise):
    h1_sinr = jam_power[0] * jam_gain[0] / noise
    h2_sinr = jam_power[1] * jam_gain[1] / noise
    return  h1_sinr, h2_sinr
        ...
        if trsan.authenticate(Car, trsan) and len(rsu) > 0 : #验证车辆是否注册，并且state是否为true,并且当前节点范围内有其他通信的rsu
            for j in range(1, 7):
                ...
                state_seq_t, select_rsu, select_jam, jam_power, delay, energy, sinr, h_sinr = \
                    initialize_select_state_seq(episode, hacking_car_id, j)
                for s in range(j):
               ...
               # 将训练得到的选择rsu节点与当前car的信息到区块中.
                ...
                for time_slot in tqdm(range(max_time_slot)):
                    ...
                    if j == 1:
                        jammer_state = jam_power[0] * jam_gain[0] / noise
                        jammer_state = [jammer_state / 10, jammer_state / 10]
                    else:
                        jam1_h_sinr, jam2_h_sinr = get_jam_to_h_sinr(jam_power, jam_gain, noise)
                    ...
                    next_state_seq_t = initialize_state_seq(select_rsu, jam_power, jam_gain, j, total_power,
                ...
                endtime = time.time()
    for h in range(6):
        np.savetxt(pwd + f"/DQN_random{h+3}_utility.txt", utilitys[h])
        #np.savetxt(pwd + f"/DQN_random_delay.txt", save_delay)
        np.savetxt(pwd + f"/DQN_random{h+3}_energy.txt", save_energy[h])
        np.savetxt(pwd + f"/DQN_random{h+3}_sinr.txt", save_sinr[h])
        np.savetxt(pwd + f"/DQN_random{h+3}_power.txt", save_power[h])
